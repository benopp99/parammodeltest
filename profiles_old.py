...
fig_Phot_all = plt.figure(figsize=(4.5,3.375))
...
fig_Zhot_all = plt.figure(figsize=(4.5,3.375))
...
#    lM_200 = Mtot[indexes_R200]
    ...
    ax_Phot_all.plot(10**lR_500, np.log10(P_cgs), color=colour, lw=4,ls='-',zorder=20-h,label=labelwrite,alpha=alpha_plot)

    if(lM_200>14.8):    
        ax_Phot_all.plot(10**lR_500, np.log10(P_Arnaud_MH150_cgs)+lTvir-lTvir_MH150, color='k', lw=2,alpha=1.0,ls=':',zorder=20+h,label="Arnaud 2010 Clusters")
        
    ax_Phot_all.plot(10**lR_500, np.log10(P_Arnaud_MH150_cgs)+lTvir-lTvir_MH150, color=colour, lw=2,alpha=1.0,ls=':',zorder=20+h)
        
    
    #ax_nHhot_all.plot(10**lR,np.log10(nDM_H)+(lM_200-12.0),color=colour,lw=1,ls=':',zorder=10-h)
    
    ax_nHhot_all.plot(10**lR, np.log10(nH_cgs)+(lM_200-12.0), color=colour, lw=4,ls='-',zorder=20-h,label=labelwrite,alpha=alpha_plot)

    ax_Zhot_all.plot(10**lR_500, lZ+1.87+0.14, color=colour, lw=4,ls='-',zorder=20-h,label=labelwrite,alpha=alpha_plot) #Asplund 0.0134

    R500_low_Lov, R500_hi_Lov, Arelax_Lov, sigrelax_Lov, Adist_Lov, sigdist_Lov = np.loadtxt("Lovisari2019_Table1.dat", usecols=(0,1,2,3,4,5),unpack=True)
    ...
    if((lM_200>13.2) & (lM_200<14.2)):    
        ax_Zhot_all.plot(10**lR_500_Lov,lArelax_Lov,color='k',ls=':',label="Lovisari 2019 relaxed")
        ax_Zhot_all.plot(10**lR_500_Lov,lAdist_Lov,color='k',ls='-.',label="Lovisari 2019 disturbed")
    ...
    ax_NOVI_all.plot(b_kpc, np.log10(N_OVI), color=colour, lw=4,ls='-',zorder=20-h,label="%s log[$N_{OVI}$] = %4.1f"%(labelwrite,np.log10(N_OVI_150_sum)),alpha=alpha_plot)
    ax_NOVII_all.plot(b_kpc, np.log10(N_OVII), color=colour, lw=4,ls='-',zorder=20-h,label="%s log[$N_{OVII}$] = %4.1f"%(labelwrite,np.log10(N_OVII_150_sum)),alpha=alpha_plot)
    ax_NOVIII_all.plot(b_kpc, np.log10(N_OVIII), color=colour, lw=4,ls='-',zorder=20-h,label="%s log[$N_{OVIII}$] = %4.1f"%(labelwrite,np.log10(N_OVIII_150_sum)),alpha=alpha_plot)
...
ax_Phot_norm.set_xlabel('$R/R_{500}$',fontsize=16)
...
ax_nHhot_norm.set_xlim(10**(lRlow+0.17),10**0.5)
...
ax_nHhot_norm.set_xlabel('$R/R_{500}$',fontsize=16)
...
ax_Phot_all.set_xlim(10**(lRlow+0.3),10**0.5)
ax_Phot_all.set_ylim(2.0,8.7)
ax_Phot_all.set_xticks([0.03,0.1,0.3,1.0,2.0])
ax_Phot_all.get_xaxis().set_major_formatter(mpl.ticker.ScalarFormatter())
ax_Phot_all.set_xlabel('$R/R_{500}$',fontsize=16)
...
fig_Phot_all.subplots_adjust(left=0.17, bottom=0.17,top=0.98,right=0.98)
...
fig_nHhot_all.subplots_adjust(left=0.17, bottom=0.17,top=0.98,right=0.98)
fig_nHhot_all.savefig('nH_hot_all.' + lsfilename + '.png')

ax_Zhot_all.set_xscale('log')
ax_Zhot_all.set_xlim(10**(lRlow+0.3),10**0.5)
ax_Zhot_all.set_ylim(-1.3,1.0)
ax_Zhot_all.set_xticks([0.03,0.1,0.3,1.0,2.0])
ax_Zhot_all.get_xaxis().set_major_formatter(mpl.ticker.ScalarFormatter())
ax_Zhot_all.set_xlabel('$R/R_{500}$',fontsize=16)
ax_Zhot_all.set_ylabel(r'log $Z/Z_{\odot}$',fontsize=16)
ax_Zhot_all.plot([1,1],[-10,10],color='k',lw=1,ls=':')
ax_Zhot_all.legend(loc="upper right", ncol=1,fontsize=10)
fig_Zhot_all.subplots_adjust(left=0.17, bottom=0.17,top=0.98,right=0.98)
fig_Zhot_all.savefig('Z_hot_all.' + lsfilename + '.png')
